...
#
# Checks immediately on start, then every 12 hours after. 
# Certbot only actually renews when the certificate is within 30 days of expiry, 
# so checking on every start is safe and cheap.
# On renewal, a deploy hook reloads nginx so it picks up the new certificate without downtime.
#
...
while true; do
    ...
    # A failed attempt must not exit the loop (set -e would otherwise kill
    # the container here); let the loop's own 12h cadence handle retries instead.
    certbot renew \
        --webroot -w /var/www/certbot \
        --deploy-hook "docker exec nginx nginx -s reload" \
        || echo "[certbot-renew] Renewal attempt failed; will retry at next check."
