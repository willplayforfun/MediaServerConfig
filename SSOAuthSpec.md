...
1. **"Dumb" browser-only web apps** (Stash, Bazarr) — no useful native auth, no native client apps. Gate these at nginx with **forward-auth** (`auth_request`). Cleanest option.
...
### Forward-auth group — Stash, Bazarr
...
- **Bazarr** ([locations.conf.tmpl:227](docker-compose.yml:227) service / nginx block to be added — Bazarr is not currently proxied in locations.conf, only port-published; **the implementer must add an nginx location for Bazarr first** if web SSO is wanted, or gate it at the published port differently). Note Bazarr's own API (used by Sonarr/Radarr integrations, if any) would break behind forward-auth — confirm none are in use.
...
1. **Forward-auth for Stash + Bazarr** (incl. adding Bazarr's nginx location). Proves the render-toggle mechanism end to end. ~0.5 day. *Highest value / least risk.*
...
4. **Bazarr web SSO:** Bazarr isn't currently nginx-proxied (port-published only). Add an nginx location for it as part of this, or leave Bazarr on LAN-only access and skip gating it?
